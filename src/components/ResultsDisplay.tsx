...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({
  ...
  const getProductLabel = (p: ProductCalculation, isSeed: boolean) => {
    ...
    return `${p.productName} – ${p.productPackageString} – ${p.applicationRate} ${getCorrectRateUnit(p.rateUnit, p.productName)}${isSeed ? ` – Treats ${capacity}` : ""}`;
  };

  const renderGridItem = (label: string, value: string | number) => (
    <div className="flex justify-between items-center min-h-[48px]">
      <span className="text-yellow-300 font-semibold whitespace-nowrap print:text-black">{label}</span>
      <span className="text-white text-right ml-4 whitespace-nowrap print:text-black">{value}</span>
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      {seedTreatmentResults.length > 0 && (
        <div className="p-4 rounded border border-gray-700 bg-zinc-800 print:bg-white print:text-black">
          <h3 className="text-xl font-bold text-blue-400 mb-4 print:text-black">Seed Treatment Calculations</h3>
          {seedTreatmentResults.map((result, index) => (
            <div key={index} className="mb-6">
              <h4 className="text-lg font-bold text-white mb-2 print:text-black">{getProductLabel(result, true)}</h4>
              <div className="grid grid-cols-2 gap-4 p-4 border border-gray-600 rounded mb-4">
                <h5 className="col-span-2 text-md font-bold text-blue-400 print:text-black">Seeding Information</h5>
              ...
              <div className="grid grid-cols-2 gap-4 p-4 border border-gray-600 rounded">
                <h5 className="col-span-2 text-md font-bold text-blue-400 print:text-black">Seed Treatment Costs</h5>
      ...
      {inFurrowFoliarResults.length > 0 && (
        <div className="p-4 rounded border border-gray-700 bg-zinc-800 print:bg-white print:text-black print-page-break">
          <h3 className="text-xl font-bold text-blue-400 mb-4 print:text-black">In-Furrow / Foliar Product Costs</h3>
          {inFurrowFoliarResults.map((result, index) => (
            <div key={index} className="mb-6">
              <h4 className="text-lg font-bold text-white mb-2 print:text-black">{getProductLabel(result, false)}</h4>
              <div className="grid grid-cols-2 gap-4 p-4 border border-gray-600 rounded">
                {renderGridItem("Application Rate:", `${formatNumber(result.applicationRate ?? 0, 2)} ${getCorrectRateUnit(result.rateUnit, result.productName)}`)}
                {renderGridItem("Total Amount of Product Needed:", formatNumber(result.totalProductNeeded ?? 0, 2))}
                {renderGridItem("Total Product Units to Order:", `${result.packagesNeeded} – ${result.productPackageString}`)}
      ...
      <div className="p-4 rounded border border-gray-700 bg-zinc-800 print:bg-white print:text-black">
        <h3 className="text-xl font-bold text-blue-400 mb-4 print:text-black">Total Program Cost</h3>
        <div className="grid grid-cols-2 gap-4 p-4 border border-gray-600 rounded">
      ...
      <div className="p-4 rounded border border-gray-700 bg-zinc-800 print:bg-white print:text-black">
        <h3 className="text-xl font-bold text-blue-400 mb-4 print:text-black">Breakeven ROI Calculation</h3>
        <div className="grid grid-cols-5 gap-4 text-center text-white print:text-black">
          <div className="bg-zinc-700 p-4 rounded print:bg-white">
            <div className="text-yellow-300 font-semibold mb-1 print:text-black">Breakeven Yield per Acre</div>
            <div>{formatNumber(breakevenYield ?? 0, 2)} {displayUnit}</div>
          </div>
          <div className="bg-zinc-700 p-4 rounded print:bg-white">
            <div className="text-yellow-300 font-semibold mb-1 print:text-black">Yield Needed for 2:1 ROI</div>
            <div>{formatNumber(roi2 ?? 0, 2)} {displayUnit}</div>
          </div>
          <div className="bg-zinc-700 p-4 rounded print:bg-white">
            <div className="text-yellow-300 font-semibold mb-1 print:text-black">Yield Needed for 3:1 ROI</div>
            <div>{formatNumber(roi3 ?? 0, 2)} {displayUnit}</div>
          </div>
          <div className="bg-zinc-700 p-4 rounded print:bg-white">
            <div className="text-yellow-300 font-semibold mb-1 print:text-black">Yield Needed for 4:1 ROI</div>
            <div>{formatNumber(roi4 ?? 0, 2)} {displayUnit}</div>
          </div>
          <div className="bg-zinc-700 p-4 rounded print:bg-white">
            <div className="text-yellow-300 font-semibold mb-1 print:text-black">Yield Needed for 5:1 ROI</div>
