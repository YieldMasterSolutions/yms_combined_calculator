    ...
    <div ref={resultRef} className="mt-6 space-y-6">
      ...
      {productsData.map((product, i) => (
        <div key={i} className="bg-gray-900 border border-gray-700 rounded-xl p-3">
          ...
          <p className="mb-1">Total Product Units to Order = {product.packagesNeeded} - {product.productPackageString}</p>
          <p className="mb-1">Total Cost to Grower (MSRP) = ${product.originalTotalCostToGrower.toFixed(2)}</p>
          <p className="mb-1">Total Discounted Cost to Grower = ${product.discountedTotalCostToGrower.toFixed(2)}</p>
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-3">
        ...
        <p>Breakeven Yield per Acre = {breakevenYield !== null ? breakevenYield.toFixed(2) : "N/A"} {cropPriceUnit}</p>
        <p>ROI Yield for 2:1 Investment = {roi2 !== null ? roi2.toFixed(2) : "N/A"} {cropPriceUnit}</p>
        <p>ROI Yield for 3:1 Investment = {roi3 !== null ? roi3.toFixed(2) : "N/A"} {cropPriceUnit}</p>
        <p>ROI Yield for 4:1 Investment = {roi4 !== null ? roi4.toFixed(2) : "N/A"} {cropPriceUnit}</p>
        <p>ROI Yield for 5:1 Investment = {roi5 !== null ? roi5.toFixed(2) : "N/A"} {cropPriceUnit}</p>
      </div>
