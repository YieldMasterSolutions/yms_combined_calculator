...
export default function CombinedCalculator() {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const selectedSeedProducts = selectedSeedTreatmentProducts.filter(p => p.product && p.product["Product Name"]).map(p => p.product);
    ...
    const selectedFoliarProducts = selectedSeedTreatmentProducts.filter(p => p.product && p.product["Product Name"] && (p.applicationMethod === "In-Furrow" || p.applicationMethod === "Foliar")).map(p => p.product);
  ...
  return (
    ...
    >
      ...
      {(seedResults.length > 0 || foliarResults.length > 0) && (
        <ResultsDisplay
          seedTreatmentResults={seedResults} inFurrowFoliarResults={foliarResults}
