  ...
  const renderProductCard = (product: ProductCalculation, isSeed: boolean) => (
    <div className="bg-white dark:bg-zinc-900 text-black dark:text-white border border-zinc-300 dark:border-zinc-700 rounded p-4">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1">
        {isSeed && (
          <>
            <div className={gridCell}><span className={label}>Total Number of Seeds to be Treated:</span><span className={value}>{formatNumber(product.totalSeeds)}</span></div>
            <div className={gridCell}><span className={label}>Total Weight of Seeds to be Treated:</span><span className={value}>{formatNumber(product.totalWeight)} lbs</span></div>
            <div className={gridCell}><span className={label}>Total Number of Units to be Treated:</span><span className={value}>{formatNumber(product.unitsToBeTreated)}</span></div>
            <div className={gridCell}><span className={label}>Number of Seeds per Unit:</span><span className={value}>{formatNumber(product.seedsPerUnit)}</span></div>
          </>
        )}
