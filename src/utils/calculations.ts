...
export function calculateSeedTreatmentData({
...
}: SeedTreatmentInput): ProductCalculation[] {
  ...
  const seedsPerUnit = seedsPerUnitOverride || seedsPerPound * lbsPerUnit;
...
export function calculateProductData(
  ...
  const discountedCostOfActualUsedProduct = (totalProductNeeded ?? 0) * (costPerUnit ?? 0) * discountFactor;

  const individualCostPerAcre = rateUnit?.includes("/acre")
    ? (applicationRate ?? 0) * (costPerUnit ?? 0) * discountFactor
    : acres > 0
    ? discountedCostOfActualUsedProduct / acres
    : 0;

  const productCostPerUnitSeed = unitsToBeTreated > 0
    ? discountedCostOfActualUsedProduct / unitsToBeTreated
    : 0;
  ...
  return {
    ...
    discountedTotalCostToGrower: discountedCostOfActualUsedProduct,
    ...
    discountedCostToGrower: discountedCostOfActualUsedProduct,
