  ...
  const renderProductCard = (product: ProductCalculation, isSeed: boolean) => (
    <div className="bg-white dark:bg-zinc-900 text-black dark:text-white border border-zinc-300 dark:border-zinc-700 rounded p-4">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1">
        ...
        <div className={gridCell}><span className={label}>Treatment Capacity per Package:</span><span className={value}>{product.treatmentCapacity} {isSeed ? "units" : "acres"}</span></div>
        <div className={gridCell}><span className={label}>Product Cost per Ounce:</span><span className={value}>${formatNumber(product.productCostPerOz)}</span></div>
        <div className={gridCell}><span className={label}>Total Cost to Grower (MSRP):</span><span className={value}>${formatNumber(product.totalCostToGrower)}</span></div>
        <div className={gridCell}><span className={label}>Total Discounted Cost to Grower:</span><span className={value}>${formatNumber(product.discountedCostToGrower)}</span></div>
        {isSeed ? (
          <>
